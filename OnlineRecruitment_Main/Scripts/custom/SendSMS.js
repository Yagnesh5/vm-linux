$(document).ready(function () {
    ...
    $(".acls_SendSMS").css("color", "white");
    $(".acls_SendSMS").mouseover(function () {
        $(".acls_SendSMS").css("color", "#6a6c6f");
    });
    $(".acls_SendSMS").mouseout(function () {
        $(".acls_SendSMS").css("color", "white");
    });
    LoadData();
    oTable = $('#tblSMSTemplates').DataTable();
...
function ajaxSuccess(data) {
    ...
    if (data.successlist[0].Message === "Inserted") {
        ...
        $("#SendSMSId").val(data.successlist[0].Data.Id).trigger('change');
        $("#SendSMSId").val(data.successlist[0].Data.Id);
...
$("#btnsubmit").on("click", function (e) {
    $("#submitform").validate({
        submitHandler: function (form) {
            ...
            params['url'] = BaseUrl + 'odata/SendMesssages';
...
$(document).on("click", ".btnedit", function () {
    ...
    var params = $.extend({}, doAjax_params_default);
    params['url'] = BaseUrl + 'odata/SendMesssages(' + id + ')';
...
function LoadData() {
    ...
    $('#tblSMSTemplates').DataTable({
        "destroy": true,
        "processing": true,
        "serverSide": true,
        "orderMulti": false,
        "ajax": {
            "url": "/Master/GetSendMessages",
            "type": "POST",
            "datatype": "json"
        },
        "columns": [
            { "data": "Name", "name": "SendSMS Templates" },
            {
                data: null,
                className: "center",
                "render": function (data, type, row, meta) {
                    var IsEditBtn = IsEdit === "True" ? '| <button class="btn btn-info btn-xs btnedit" type="button" title="Edit Send SMS" id="" data-data="' + data.Id + '"><i class="fa fa-edit"></i> </button>' : "";
